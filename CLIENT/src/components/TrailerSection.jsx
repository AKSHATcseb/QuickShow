...
const TrailerSection = () => {
  ...
  return (
    <div className='px-6 md:px-16 lg:px-24 xl:px-44 py-20 overflow-hidden font-outfit ' >
      <p className='text-gray-300 font-medium text-lg max-w-[960px] mx-auto'>Trailers</p>
      <div className='relative mt-6'>
        <BlurCircle top='-100px' right='-100px' />

      <ReactPlayer src={currentTrailer.videoUrl} controls={false} className='rounded-lg w-[770px] h-[400px] mx-auto max-w-full' width="960" height="540" />
      </div>
      ...
      <div className='group grid grid-cols-4 gap-4 md:gap-8 mt-8 max-w-3xl mx-auto'>
        {dummyTrailers.map((trailer) => (
          <div key={trailer.image} onClick={() => {setCurrentTrailer(trailer)}} className='relative group-hover:not-hover:opacity-50 hover:-translate-y-1 duration-300 transition max-md:h-60 md:max-h-60 cursor-pointer'>
            <img className='rounded-lg w-full h-full object-cover brightness-75' src={trailer.image} alt="trailer.." />
